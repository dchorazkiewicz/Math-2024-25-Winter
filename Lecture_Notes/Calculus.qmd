...
&=\lim_{h \to 0} \frac{sin(a)(cos(h)-1)+cos(a)sin(h)}{h}\\
&=cos(a)\lim_{h \to 0} \frac{sin(h)}{h}\\
&=cos(a)
\end{align*}
$$
...
$$(x^4+3x^2+1)'=(x^4)' + (3x^2)' + (1)'=4x^3+6x$$

##### Second example

$$(cos(x)e^x)'=(cos(x))'e^x+cos(x)(e^x)'=-sin(x)e^x+cos(x)e^x=(cos(x)-sin(x))e^x$$

##### Third example
More coplicated example: 
...
This is very important rule that allows us to differentiate composite functions. If $f(x)=g(h(x))$, then
$$
f'(x)=g'(h(x))h'(x)
$$
...
* $g(y)=\sin(y) \Rightarrow g'(y)=\cos(y)$
